...
const getAgencies = (agencyId?: string) => {
  return gtfsDB.selectDistinct({
...
const gtfsGetRouteByAgencySchema = z.object({
  agencyId: z.string().trim().max(32).optional(),
});
...
export default defineEventHandler(async (event) => {
  ...
  const { agencyId } = await getValidatedQuery(event, gtfsGetRouteByAgencySchema.parse);

  try {
    return getAgencies(agencyId);
  } catch(err:any) {
    throw createError({
      statusCode: 500,
    });
