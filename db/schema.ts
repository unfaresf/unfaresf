...
export const users = sqliteTable("users", {
  ...
  roles: text()
    .notNull()
    .default(JSON.stringify([Roles.Editor])),
});
...
export type GetUser = Prettify<
  Omit<InferSelectModel<typeof users>, "roles"> & { roles: Roles[] }
>;
...
export const broadcasts = sqliteTable("broadcasts", {
  ...
  reportId: integer("report_id", { mode: "number" })
    .unique()
...
export const integrationOptionsSchema = z.discriminatedUnion("type", [
  mapIntegrationOptionSchema,
  mastodonIntegrationOptionSchema,
  twitterIntegrationOptionSchema,
  bskyIntegrationOptionSchema,
]);
